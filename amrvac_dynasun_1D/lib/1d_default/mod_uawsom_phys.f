...
!! end rad cool
      ...
      if(primitive) then
        ...
        if(uawsom_internal_e) then
          ...
          tmp(ixOmin1:ixOmax1)=w(ixOmin1:ixOmax1,e_)-uawsom_kin_en(w,ixImin1,&
             ...
             ixOmax1)-uawsom_wA_en(w,ixImin1,ixImax1,ixOmin1,ixOmax1) !Max: Added AW + kink
          if(has_equi_pe0) then
            tmp(ixOmin1:ixOmax1) = tmp(ixOmin1:ixOmax1)+&
               block%equi_vars(ixOmin1:ixOmax1,equi_pe0_,0)*inv_gamma_1
          endif
  ...
  subroutine uawsom_ei_to_e(ixImin1,ixImax1,ixOmin1,ixOmax1,w,x)
    ...
    w(ixImin1:ixImax1,e_)=w(ixImin1:ixImax1,e_)+uawsom_kin_en(w,ixImin1,&
       ...
       ixImin1,ixImax1,ixImin1,ixImax1) !Max: Added AW + kink
  ...
  subroutine uawsom_get_temperature_from_etot(w, x, ixImin1,ixImax1, ixOmin1,&
    ...
    res(ixOmin1:ixOmax1)=(gamma_1*(w(ixOmin1:ixOmax1,e_)- uawsom_kin_en(w,&
       ...
       ixOmax1))/w(ixOmin1:ixOmax1,rho_) !Max: added AW + Kink
  ...
  subroutine uawsom_get_flux(wC,w,x,ixImin1,ixImax1,ixOmin1,ixOmax1,idim,f)
    ...
    if(uawsom_energy) then 
      ptotal(ixOmin1:ixOmax1)=w(ixOmin1:ixOmax1,&
         ...
         wAplus_) + w(ixOmin1:ixOmax1,wAminus_))/2.0d0 !Max: Added AW + Kink pressure
    else
  ...
  subroutine uawsom_get_flux_split(wC,w,x,ixImin1,ixImax1,ixOmin1,ixOmax1,idim,&
    ...
    ptotal(ixOmin1:ixOmax1)=pgas(ixOmin1:ixOmax1)+0.5d0*sum(w(ixOmin1:ixOmax1,&
       ...
       wkplus_) + w(ixOmin1:ixOmax1,wkminus_))/4.d0+(w(ixOmin1:ixOmax1,&
       wAplus_) + w(ixOmin1:ixOmax1,wAminus_))/2.d0 !Max: added AW + kink pressure
  ...
  subroutine w_add_source(qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,wCT,w,x,energy,&
    ...
    logical, intent(inout)          :: active
    double precision                :: v(ixImin1:ixImax1,1:ndir),&
        Bmag(ixImin1:ixImax1), B0(ixImin1:ixImax1,3), Bpo, Bzo, y0, kx
    double precision                :: divv(ixImin1:ixImax1),&
       Lperp(ixImin1:ixImax1),Lperp_AW(ixImin1:ixImax1),&
       Lperp_AWCD004(ixImin1:ixImax1),Lperp_AWCD007(ixImin1:ixImax1),&
    ...
    double precision                :: vA(ixImin1:ixImax1),&
       vA2(ixImin1:ixImax1),vA3(ixImin1:ixImax1),gradvA(ixImin1:ixImax1),&
       gradvA2(ixImin1:ixImax1),gradvA3(ixImin1:ixImax1),&
       refvA(ixImin1:ixImax1),lnvA(ixImin1:ixImax1),lnvA2(ixImin1:ixImax1),&
       lnvA3(ixImin1:ixImax1), gradlnvA(ixImin1:ixImax1) ,&
        gradlnvA2(ixImin1:ixImax1) , gradlnvA3(ixImin1:ixImax1) 
    double precision                :: vk(ixImin1:ixImax1),&
       vk2(ixImin1:ixImax1),vk3(ixImin1:ixImax1),gradvk(ixImin1:ixImax1),&
       gradvk2(ixImin1:ixImax1),gradvk3(ixImin1:ixImax1),&
       refvk(ixImin1:ixImax1),lnvk(ixImin1:ixImax1),lnvk2(ixImin1:ixImax1),&
       lnvk3(ixImin1:ixImax1), gradlnvk(ixImin1:ixImax1) ,&
        gradlnvk2(ixImin1:ixImax1) , gradlnvk3(ixImin1:ixImax1) 
    integer                         :: ix1, ix2, ix3
    ...
    !> Used later for testing using correct mag field
    if(B0field) then
      B(ixImin1:ixImax1,1:ndir)=w(ixImin1:ixImax1,&
         mag(1:ndir))+block%B0(ixImin1:ixImax1,1:ndir,0) !> Magnetic field pertubations change the radius of flux tube
    else
      B(ixImin1:ixImax1,1:ndir)=w(ixImin1:ixImax1,mag(1:ndir)) 
    end if

    radius(ixOmin1:ixOmax1) = 1.d8/unit_length * &
       ((Busr/unit_magneticfield)/(B(ixOmin1:ixOmax1,1)))**0.5d0 !Radius = R_0*(B0/B)0.5 TVD 2025 paper uses R_0 = 1Mm (1e8 cm) > Used for 1D simulations 

    if(qsourcesplit .eqv. .false.) then
      active = .true.
      call uawsom_get_v(wCT,x,ixImin1,ixImax1,ixImin1,ixImax1,v)

      !> Used as temp was used in LPerpAW
      ...
      Lperp_AW(ixOmin1:ixOmax1) = (1.0d10/6.961d10) * (1.0d0 / &
         B(ixOmin1:ixOmax1,1))**0.5d0 
      ...
      Gamma_plus(ixOmin1:ixOmax1) = (2.0d0 / Lperp_AW(ixOmin1:ixOmax1)) * &
         (wCT(ixOmin1:ixOmax1, wAminus_)/wCT(ixOmin1:ixOmax1,rho_))**0.5d0 !> With wCT array rather than w
      Gamma_minus(ixOmin1:ixOmax1) = (2.0d0 / Lperp_AW(ixOmin1:ixOmax1)) * &
         (wCT(ixOmin1:ixOmax1, wAplus_)/wCT(ixOmin1:ixOmax1,rho_))**0.5d0
      ...
      vA(ixImin1:ixImax1) = B(ixImin1:ixImax1,1)/dsqrt(w(ixImin1:ixImax1,&
         rho_))
      lnvA(ixImin1:ixImax1) = log(vA(ixImin1:ixImax1))
      
      call gradient(lnvA,ixImin1,ixImax1,ixOmin1,ixOmax1,1,gradlnvA)
      ...
      do ix1 = ixOmin1, ixOmax1 
        w(ix1,wAminus_) = w(ix1,wAminus_) - qdt*(divv(ix1)*wCT(ix1,&
           wAminus_)/2.0d0 + wCT(ix1,wAminus_)*Gamma_minus(ix1) + ((wCT(ix1,&
           mom(1)) + vA(ix1))/vA(ix1))*refvA(ix1)*wCT(ix1,wAminus_))
        w(ix1,wAplus_)  = w(ix1,wAplus_)  - qdt*(divv(ix1)*wCT(ix1,&
           wAplus_)/2.0d0 + wCT(ix1,wAplus_)*Gamma_plus(ix1) - ((wCT(ix1,&
           mom(1)) + vA(ix1))/vA(ix1))*refvA(ix1)*wCT(ix1,wAminus_))
      end do
      ...
      w(ixOmin1:ixOmax1,wkplus_)  = w(ixOmin1:ixOmax1,&
         ...
         1.d0))**0.5d0/Lperp(ixOmin1:ixOmax1)) !+ refvk(ixOmin1:ixOmax1)*wCT(ixOmin1:ixOmax1,wkminus_) - refvk(ixOmin1:ixOmax1)*wCT(ixOmin1:ixOmax1,wkplus_))
      w(ixOmin1:ixOmax1,wkminus_) = w(ixOmin1:ixOmax1,&
         ...
         rho_)*(1+ff*zeta(ixOmin1:ixOmax1)-ff)**(-&
         1.d0))**0.5d0/Lperp(ixOmin1:ixOmax1)) !- refvk(ixOmin1:ixOmax1)*wCT(ixOmin1:ixOmax1,wkminus_) + refvk(ixOmin1:ixOmax1)*wCT(ixOmin1:ixOmax1,wkminus_))
