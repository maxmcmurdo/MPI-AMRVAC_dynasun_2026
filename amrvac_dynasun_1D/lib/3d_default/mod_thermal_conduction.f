...
contains
  ...
  subroutine sts_set_source_tc_mhd(ixImin1,ixImin2,ixImin3,ixImax1,ixImax2,&
    ...
    !  if (abs(wres(ix1,fl%e_)) > 2.d0) then
        !if (mype == 7) then      
        !  write(*,*) "it                 = ", it
    ...
    !if (mod(it,5000) == 0 .and. mype == 3) then
    !  write(*,*) "max TC 4th block = ", maxval(abs(wres(ixO^S,fl%e_)))
    !  write(*,*) "min TC first block = ", minval(wres(ixO^S,fl%e_))
    !end if

    !if (mod(it,5000) == 0 .and. mype == 4) then
    !  write(*,*) "max TC 5th block = ", maxval(abs(wres(ixO^S,fl%e_)))
