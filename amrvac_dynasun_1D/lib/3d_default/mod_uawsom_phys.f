...
!! end rad cool
  ...
  subroutine w_add_source(qdt,ixImin1,ixImin2,ixImin3,ixImax1,ixImax2,ixImax3,&
    ...
    double precision                :: divv(ixImin1:ixImax1,ixImin2:ixImax2,&
       ...
       Lperp_AWCD004(ixImin1:ixImax1,ixImin2:ixImax2,ixImin3:ixImax3),&
       Lperp_AWCD007(ixImin1:ixImax1,ixImin2:ixImax2,ixImin3:ixImax3),&
    ...
    double precision                :: vk(ixImin1:ixImax1,ixImin2:ixImax2,&
       ...
       lnvk3(ixImin1:ixImax1,ixImin2:ixImax2,ixImin3:ixImax3),&
        gradlnvk(ixImin1:ixImax1,ixImin2:ixImax2,ixImin3:ixImax3) ,&
    ...
    radius(ixOmin1:ixOmax1,ixOmin2:ixOmax2,&
       ...
       ((Busr/unit_magneticfield)/(B(ixOmin1:ixOmax1,ixOmin2:ixOmax2,&
       ixOmin3:ixOmax3,1)))**0.5d0 !Radius = R_0*(B0/B)0.5,R_0*(B0/B)0.5,R_0*(B0/B)0.5 TVD 2025 paper uses R_0 = 1Mm (1e8 cm) > Used for 1D simulations 
    ...
    Lperp_AW(ixOmin1:ixOmax1,ixOmin2:ixOmax2,&
       ixOmin3:ixOmax3) = (1/1.d8) * 1.5d5 * 1.0d2 * (Tmp(ixOmin1:ixOmax1,&
       ixOmin2:ixOmax2,ixOmin3:ixOmax3) / B(ixOmin1:ixOmax1,ixOmin2:ixOmax2,&
       ixOmin3:ixOmax3,1))**0.5d0 !> Fine in 1D VDH14 implementation

    !> (1/unit_length) is required as 1.5d7 is in cm and 10^2 is taken from sqrt(B tesla) = sqrt(10**-4*B Gauss) = 10**-2*sqrt(B_Gauss) TVD+2014. Linear conversion to Gauss so no extras needed still dimensionless
    !Lperp_AW(ixO^S) = 6.961d0*(1/6.961d10) * 1.5d5 * 1.0d2 * 1.0d2 * (Tmp(ixO^S)/B(ixO^S,1))**0.5d0

    !Lperp_AWCD004(ixO^S) = 0.04d0*(20.0d0/B(ixO^S,1))**0.5d0 !> Cooper Downs Lperp_AW (they performed parameter study varying 0.01 - 0.07 to control AW damping)
    !Lperp_AWCD007(ixO^S) = 0.07d0*(20.0d0/B(ixO^S,1))**0.5d0 !> Cooper Downs Lperp_AW (they performed parameter study varying 0.01 - 0.07 to control AW damping)

    !if (mod(it,10000)==0 .and. mype==0) then
    !  write(*,*)"LperpAW  (Mm) = ", Lperp_AW(ixOmin1)*696.1
    !  write(*,*)"Lperpkk  (Mm) = ", Lperp(ixOmin1)*696.1
    !  write(*,*)"Lperp001 (Mm) = ", Lperp_AWCD004(ixOmin1)*696.1
    !  write(*,*)"Lperp007 (Mm) = ", Lperp_AWCD007(ixOmin1)*696.1
    !end if
    ...
    Gamma_plus(ixOmin1:ixOmax1,ixOmin2:ixOmax2,&
       ...
       ixOmin3:ixOmax3,rho_))**0.5d0 !> With wCT array rather than w
    ...
    vA(ixImin1:ixImax1,ixImin2:ixImax2,ixImin3:ixImax3) = B(ixImin1:ixImax1,&
       ixImin2:ixImax2,ixImin3:ixImax3,1)/dsqrt(w(ixImin1:ixImax1,&
    ...
    lnvA(ixImin1:ixImax1,ixImin2:ixImax2,&
       ...
       ixImin3:ixImax3))
    call gradient(lnvA,ixImin1,ixImin2,ixImin3,ixImax1,ixImax2,ixImax3,ixOmin1,&
       ixOmin2,ixOmin3,ixOmax1,ixOmax2,ixOmax3,ndim,gradlnvA)
    ...
    !  refvA(ix1) = min(vA(ix1)*gradlnvA(ix1), max(Gamma_plus(ix1),Gamma_minus(ix1)))    !> Inspiration from VDH 2014, not used in 1D McMurdo+2025
      refvA(ix1) = 0.5d0*vA(ix1)*gradlnvA(ix1) !> Note vA*grad(ln(vA)) = gradvA in 1D used in McMurdo+2025
    end do
    ...
    do ix1 = ixOmin1, ixOmax1 
      w(ix1,ixOmin2:ixOmax2,ixOmin3:ixOmax3,wAminus_) = w(ix1,ixOmin2:ixOmax2,&
         ixOmin3:ixOmax3,wAminus_) - qdt*(divv(ix1,ixOmin2:ixOmax2,&
         ixOmin3:ixOmax3)*wCT(ix1,ixOmin2:ixOmax2,ixOmin3:ixOmax3,&
         wAminus_)/2.0d0 + wCT(ix1,ixOmin2:ixOmax2,ixOmin3:ixOmax3,&
         wAminus_)*Gamma_minus(ix1,ixOmin2:ixOmax2,&
         ixOmin3:ixOmax3) + ((wCT(ix1,ixOmin2:ixOmax2,ixOmin3:ixOmax3,&
         mom(1)) + vA(ix1,ixOmin2:ixOmax2,ixOmin3:ixOmax3))/vA(ix1,&
         ixOmin2:ixOmax2,ixOmin3:ixOmax3))*refvA(ix1,ixOmin2:ixOmax2,&
         ixOmin3:ixOmax3)*wCT(ix1,ixOmin2:ixOmax2,ixOmin3:ixOmax3,wAminus_))
      w(ix1,ixOmin2:ixOmax2,ixOmin3:ixOmax3,wAplus_)  = w(ix1,ixOmin2:ixOmax2,&
         ixOmin3:ixOmax3,wAplus_)  - qdt*(divv(ix1,ixOmin2:ixOmax2,&
         ixOmin3:ixOmax3)*wCT(ix1,ixOmin2:ixOmax2,ixOmin3:ixOmax3,&
         wAplus_)/2.0d0 + wCT(ix1,ixOmin2:ixOmax2,ixOmin3:ixOmax3,&
         wAplus_)*Gamma_plus(ix1,ixOmin2:ixOmax2,ixOmin3:ixOmax3) - ((wCT(ix1,&
         ixOmin2:ixOmax2,ixOmin3:ixOmax3,mom(1)) + vA(ix1,ixOmin2:ixOmax2,&
         ixOmin3:ixOmax3))/vA(ix1,ixOmin2:ixOmax2,ixOmin3:ixOmax3))*refvA(ix1,&
         ixOmin2:ixOmax2,ixOmin3:ixOmax3)*wCT(ix1,ixOmin2:ixOmax2,&
         ixOmin3:ixOmax3,wAminus_))
    end do
    ...
    w(ixOmin1:ixOmax1,ixOmin2:ixOmax2,ixOmin3:ixOmax3,&
       ...
       ixOmin2:ixOmax2,ixOmin3:ixOmax3)) !+ refvk(ixOmin1:ixOmax1,ixOmin2:ixOmax2,ixOmin3:ixOmax3)*wCT(ixOmin1:ixOmax1,ixOmin2:ixOmax2,ixOmin3:ixOmax3,wkminus_) - refvk(ixOmin1:ixOmax1,ixOmin2:ixOmax2,ixOmin3:ixOmax3)*wCT(ixOmin1:ixOmax1,ixOmin2:ixOmax2,ixOmin3:ixOmax3,wkplus_))
    w(ixOmin1:ixOmax1,ixOmin2:ixOmax2,ixOmin3:ixOmax3,&
       wkminus_) = w(ixOmin1:ixOmax1,ixOmin2:ixOmax2,ixOmin3:ixOmax3,&
       ...
       ixOmin3:ixOmax3,rho_)*(1+ff*zeta(ixOmin1:ixOmax1,ixOmin2:ixOmax2,&
       ixOmin3:ixOmax3)-ff)**(-1.d0))**0.5d0/Lperp(ixOmin1:ixOmax1,&
       ixOmin2:ixOmax2,ixOmin3:ixOmax3)) !- refvk(ixOmin1:ixOmax1,ixOmin2:ixOmax2,ixOmin3:ixOmax3)*wCT(ixOmin1:ixOmax1,ixOmin2:ixOmax2,ixOmin3:ixOmax3,wkminus_) + refvk(ixOmin1:ixOmax1,ixOmin2:ixOmax2,ixOmin3:ixOmax3)*wCT(ixOmin1:ixOmax1,ixOmin2:ixOmax2,ixOmin3:ixOmax3,wkminus_))
  ...
  subroutine get_zeta(w,x,ixImin1,ixImin2,ixImin3,ixImax1,ixImax2,ixImax3,&
    ...
    zeta(ixImin1:ixImax1,ixImin2:ixImax2,&
       ixImin3:ixImax3) = (zeta0-1.d0)*exp(-(x(ixImin1:ixImax1,ixImin2:ixImax2,&
       ixImin3:ixImax3,1)-xprobmin1)/5.d0)+1.d0 !> MAX: Open field MAX TODO: Should we have a better representation for this expansion with distance along a field line rather than just height? (for 2D+)
