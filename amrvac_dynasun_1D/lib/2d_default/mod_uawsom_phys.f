...
!! end rad cool
  ...
  subroutine w_add_source(qdt,ixImin1,ixImin2,ixImax1,ixImax2,ixOmin1,ixOmin2,&
    ...
    logical, intent(inout)          :: active
    double precision                :: v(ixImin1:ixImax1,ixImin2:ixImax2,&
       1:ndir), Bmag(ixOmin1:ixOmax1,ixOmin2:ixOmax2), B0(ixImin1:ixImax1,&
       ixImin2:ixImax2,3), Bpo, Bzo, y0, kx
    double precision                :: divv(ixImin1:ixImax1,ixImin2:ixImax2),&
    ...
    Bpo=Busr/unit_magneticfield
    Bzo=Busr/unit_magneticfield
    y0 = -0.4d0 !=4Mm as in Zhang+2019
    kx=dpi/(xprobmax1-xprobmin1)

    !B0(ixI^S,1)=Bpo*dcos(kx*x(ixI^S,1))*dexp(-kx*(x(ixI^S,2)-y0)) - Bpo*dcos(3*kx*x(ixI^S,1))*dexp(-3*kx*(x(ixI^S,2)-y0))
    !B0(ixI^S,2)=-Bpo*dsin(kx*x(ixI^S,1))*dexp(-kx*(x(ixI^S,2)-y0)) + Bpo*dsin(3*kx*x(ixI^S,1))*dexp(-3*kx*(x(ixI^S,2)-y0))
    !B0(ixI^S,3)=Bzo

    if(B0field) then
      B(ixImin1:ixImax1,ixImin2:ixImax2,1:ndir)=w(ixImin1:ixImax1,&
         ixImin2:ixImax2,mag(1:ndir))+block%B0(ixImin1:ixImax1,ixImin2:ixImax2,&
         1:ndir,ndim) !> Magnetic field pertubations change the radius of flux tube
    else
      B(ixImin1:ixImax1,ixImin2:ixImax2,1:ndir)=w(ixImin1:ixImax1,&
    ...
    Lperp_AW(ixOmin1:ixOmax1,ixOmin2:ixOmax2) = (1/unit_length*10**2) * 1.5d5 &
       * 1.0d2 * (Tmp(ixOmin1:ixOmax1,ixOmin2:ixOmax2) / ((B(ixOmin1:ixOmax1,&
       ixOmin2:ixOmax2,1)**2.d0 + B(ixOmin1:ixOmax1,ixOmin2:ixOmax2,&
       2)**2 + B(ixOmin1:ixOmax1,ixOmin2:ixOmax2,3)**2)**0.5d0))**0.5d0
    ...
    !> AW evolution no reflection (1,2,3D)
    ...
    !> Reflection 1D working
    !do ix1 = ixImin1,ixImin1+1 !> Max TODO: How should magnetic field be set in ghost cells?
    !  B(ix1,1) = B(ixOmin1,1)  !> This allows for non-spurious gradients from B = 0 in ghost cells 
    !end do
    !do ix1 = ixImax1-1,ixImax1  
    !  B(ix1,1) = B(ixOmax1,1)
    !end do
    ! output Alfven wave speed B/sqrt(rho) 1D
    ! vA(ixI^S) = B(ixI^S,1)/dsqrt(w(ixI^S,rho_))
    !lnvA(ixI^S) = log(vA(ixI^S))
    !call gradient(lnvA,ixI^L,ixO^L,ndim,gradlnvA)

    !do ix1 = ixOmin1,ixOmax1
    !  refvA(ix1) = min(vA(ix1)*gradlnvA(ix1), max(Gamma_plus(ix1),Gamma_minus(ix1)))    !> Inspiration from VDH 2014, not used in 1D McMurdo+2025
    !  refvA(ix1) = vA(ix1)*gradlnvA(ix1)*0.5d0                                          !> Note vA*grad(ln(vA)) = gradvA in 1D used in McMurdo+2025
    !end do
  
    !> Single reflection ie waves cannot be re-reflected as per VDH2014 with moving frame taken into account (large upflows increase reflection) 1D open field region simulations
    !do ix1 = ixOmin1, ixOmax1 
    ...
    !> MAX TODO: Reflection for 1D loops
    !do ix1 = ixOmin1, ixOmax1 
    !  w(ix1^%1ixO^S,wAminus_) = w(ix1^%1ixO^S,wAminus_) - qdt*(divv(ix1^%1ixO^S)*wCT(ix1^%1ixO^S,wAminus_)/2.0d0 + wCT(ix1^%1ixO^S,wAminus_)*Gamma_minus(ix1^%1ixO^S) + take_off_wAminus - add_on_wAplus)
    !  w(ix1^%1ixO^S,wAplus_)  = w(ix1^%1ixO^S,wAplus_)  - qdt*(divv(ix1^%1ixO^S)*wCT(ix1^%1ixO^S,wAplus_)/2.0d0 + wCT(ix1^%1ixO^S,wAplus_)*Gamma_plus(ix1^%1ixO^S)    + take_off_wAplus  - add_on_wAminus)
    !end do

    !>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    !> MAX: Been having issues with B in ghost cells not being set 
    !>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

    !> In 1D just set it to zero gradient as not interested in refleciton at boundaries
    ...
    !> 2D equivalent 
    !> Warning: This will turn off reflection at grid boundaries need to extrapolate. 
    !>          This shows up in kink reflection clearly. Should probably set this in the mod_usr.t file
    !do ix1 = ixImin1,ixImin1+1 
    !  B(ix1^%1ixO^S,1) = B(ixOmin1^%1ixO^S,1)
    !end do
    !do ix1 = ixImax1-1,ixImax1  
    !  B(ix1^%1ixO^S,1) = B(ixOmax1^%1ixO^S,1)
    !end do 
    !do ix2 = ixImin2,ixImin2+1 
    !  B(ix2^%2ixO^S,2) = B(ixOmin2^%2ixO^S,2)
    !end do
    !do ix2 = ixImax2-1,ixImax2  
    !  B(ix2^%2ixO^S,2) = B(ixOmax2^%1ixO^S,2)
    !end do 

    do ix1 = ixImin1,ixImin1+1
      if(B0field) then
        B(ix1,ixOmin2:ixOmax2,1)=w(ix1,ixOmin2:ixOmax2,&
           mag(1))+Bpo*dcos(kx*x(ix1,ixOmin2:ixOmax2,1))*dexp(-kx*(x(ix1,&
           ixOmin2:ixOmax2,2)-y0)) - Bpo*dcos(3*kx*x(ix1,ixOmin2:ixOmax2,&
           1))*dexp(-3*kx*(x(ix1,ixOmin2:ixOmax2,2)-y0))
        B(ix1,ixOmin2:ixOmax2,2)=-Bpo*dsin(kx*x(ix1,ixOmin2:ixOmax2,&
           1))*dexp(-kx*(x(ix1,ixOmin2:ixOmax2,2)-y0)) + Bpo*dsin(3*kx*x(ix1,&
           ixOmin2:ixOmax2,1))*dexp(-3*kx*(x(ix1,ixOmin2:ixOmax2,2)-y0))
        B(ix1,ixOmin2:ixOmax2,3)=Bzo
      else
        B(ix1,ixOmin2:ixOmax2,1)=w(ix1,ixOmin2:ixOmax2,mag(1)) 
        B(ix1,ixOmin2:ixOmax2,2)=w(ix1,ixOmin2:ixOmax2,mag(2)) 
        B(ix1,ixOmin2:ixOmax2,3)=w(ix1,ixOmin2:ixOmax2,mag(3)) 
      end if
    end do
    ...
    do ix2 = ixImin2,ixImin2+1 
      if(B0field) then
        B(ixOmin1:ixOmax1,ix2,1) = w(ixOmin1:ixOmax1,ix2,&
           mag(1))+Bpo*dcos(kx*x(ixOmin1:ixOmax1,ix2,&
           1))*dexp(-kx*(x(ixOmin1:ixOmax1,ix2,&
           2)-y0)) - Bpo*dcos(3*kx*x(ixOmin1:ixOmax1,ix2,&
           1))*dexp(-3*kx*(x(ixOmin1:ixOmax1,ix2,2)-y0))
        B(ixOmin1:ixOmax1,ix2,2) = -Bpo*dsin(kx*x(ixOmin1:ixOmax1,ix2,&
           1))*dexp(-kx*(x(ixOmin1:ixOmax1,ix2,&
           2)-y0)) + Bpo*dsin(3*kx*x(ixOmin1:ixOmax1,ix2,&
           1))*dexp(-3*kx*(x(ixOmin1:ixOmax1,ix2,2)-y0))
        B(ixOmin1:ixOmax1,ix2,3) = Bzo
      else
        B(ixOmin1:ixOmax1,ix2,1)=w(ixOmin1:ixOmax1,ix2,mag(1)) 
        B(ixOmin1:ixOmax1,ix2,2)=w(ixOmin1:ixOmax1,ix2,mag(2)) 
        B(ixOmin1:ixOmax1,ix2,3)=w(ixOmin1:ixOmax1,ix2,mag(3)) 
      end if
    end do
    ...
    !>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    !> MAX: Adding reflection for 2D AWs
    !>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

    !> 2.5D simulations have constant magnetic field in idim=3 so doesnt affect reflection since gradient = 0
    ...
    vA1(ixImin1:ixImax1,ixImin2:ixImax2) = abs(B(ixImin1:ixImax1,&
       ixImin2:ixImax2,1))/dsqrt(w(ixImin1:ixImax1,ixImin2:ixImax2,rho_)) !> vA is dependent on B0+b, abs ensures non-negative argument of log
    vA2(ixImin1:ixImax1,ixImin2:ixImax2) = abs(B(ixImin1:ixImax1,&
       ixImin2:ixImax2,2))/dsqrt(w(ixImin1:ixImax1,ixImin2:ixImax2,rho_)) !> +/- vA is taken care of in flux of wave energies

    lnvA1(ixImin1:ixImax1,ixImin2:ixImax2) = log(vA1(ixImin1:ixImax1,&
       ixImin2:ixImax2))
    lnvA2(ixImin1:ixImax1,ixImin2:ixImax2) = log(vA2(ixImin1:ixImax1,&
       ixImin2:ixImax2))
    ...
    call gradient(lnvA1,ixImin1,ixImin2,ixImax1,ixImax2,ixOmin1,ixOmin2,&
       ixOmax1,ixOmax2,1,gradlnvA1)
    call gradient(lnvA2,ixImin1,ixImin2,ixImax1,ixImax2,ixOmin1,ixOmin2,&
       ixOmax1,ixOmax2,2,gradlnvA2)
    ...
    refvA(ixOmin1:ixOmax1,ixOmin2:ixOmax2) = 0.5d0*dsqrt(((((B(ixOmin1:ixOmax1,&
       ixOmin2:ixOmax2,1)/Bmag(ixOmin1:ixOmax1,&
       ixOmin2:ixOmax2))*w(ixOmin1:ixOmax1,ixOmin2:ixOmax2,&
       mom(1)))+vA1(ixOmin1:ixOmax1,ixOmin2:ixOmax2))*gradlnvA1(&
       ixOmin1:ixOmax1,ixOmin2:ixOmax2))**2.d0+((((B(ixOmin1:ixOmax1,&
       ixOmin2:ixOmax2,2)/Bmag(ixOmin1:ixOmax1,&
       ixOmin2:ixOmax2))*w(ixOmin1:ixOmax1,ixOmin2:ixOmax2,&
       mom(2)))+vA2(ixOmin1:ixOmax1,ixOmin2:ixOmax2))*gradlnvA2(&
       ixOmin1:ixOmax1,ixOmin2:ixOmax2))**2.d0)*(1+tanh((x(ixOmin1:ixOmax1,&
       ixOmin2:ixOmax2,2)-0.5d0)/0.1d0))

    !> Filter to check NanNs
    do ix1=ixOmin1,ixOmax1  
      do ix2=ixOmin2,ixOmax2
        if((ieee_is_nan(refvA(ix1,ix2))))then
          refvA(ix1,ix2) = 0.d0
          !write(*,*)"refvA = NaN at it = ", it
          !write(*,*)"x =                 ", x(ix1,ix2,1)
          !write(*,*)"x =                 ", x(ix1,ix2,2)
        end if
      end do
    ...
    !> Filter to check reflection for large values
    do ix1=ixOmin1,ixOmax1  
      do ix2=ixOmin2,ixOmax2
        if (refvA(ix1,ix2) > 1.d5)then
          !refvA(ix1,ix2)=0.d0
          write(*,*)"refvA > 1d5 at it = ", it
          write(*,*)"refvA =             ", refvA(ix1,ix2)
          write(*,*)"vA1 =               ", vA1(ix1-1:ix1:1,ix2-1:ix2+1)
          write(*,*)"vA2 =               ", vA2(ix1-1:ix1:1,ix2-1:ix2+1)
          write(*,*)"lnvA1 =             ", lnvA1(ix1-1:ix1:1,ix2-1:ix2+1)
          write(*,*)"lnvA2 =             ", lnvA2(ix1-1:ix1:1,ix2-1:ix2+1)
          !write(*,*)"x =                 ", x(ix1,ix2,1)
          !write(*,*)"x =                 ", x(ix1,ix2,2)
        end if
      end do
    ...
    !do ix1 = ixOmin1,ixOmax1
    !  refvA(ix1^%1ixO^S) = dot_product(B(ix1^%1ixO^S,1)/Bmag(ix1^%1ixO^S),w(ix1^%1ixO^S,mom(1))) !> Check that dot_product actually works
    !end do

    !if(mod(it,200)==0)then
    !  write(*,*)"-------------------------------------------------------------------------"
    !  write(*,*)"gradlnvA2(ixO^S) = ", gradlnvA2(ixOmin1:ixOmin1+1,ixOmin2:ixOmin2+1)
    !end if
                            
    !> Here reflection from one side is as important as the other like TODO for 1D loops
    do ix1=ixOmin1,ixOmax1
      do ix2=ixOmin2,ixOmax2
        w(ix1,ix2,wAminus_) = w(ix1,ix2,wAminus_) - qdt*(divv(ix1,ix2)*wCT(ix1,&
           ix2,wAminus_)/2.0d0 + wCT(ix1,ix2,wAminus_)*Gamma_minus(ix1,&
           ix2) + refvA(ix1,ix2)*wCT(ix1,ix2,wAminus_) - refvA(ix1,&
           ix2)*wCT(ix1,ix2,wAplus_))
        w(ix1,ix2,wAplus_)  = w(ix1,ix2,wAplus_)  - qdt*(divv(ix1,ix2)*wCT(ix1,&
           ix2,wAplus_)/2.0d0  + wCT(ix1,ix2,wAplus_)*Gamma_plus(ix1,&
           ix2)   - refvA(ix1,ix2)*wCT(ix1,ix2,wAminus_) + refvA(ix1,&
           ix2)*wCT(ix1,ix2,wAminus_))
      end do
    end do

    !> AW evolution no reflection (1,2,3D)
    !w(ixO^S,wAminus_) = w(ixO^S,wAminus_) - qdt*(divv(ixO^S)*wCT(ixO^S,wAminus_)/2.0d0 + wCT(ixO^S,wAminus_)*Gamma_minus(ixO^S) + refvA(ixO^S)*wCT(ixO^S,wAminus_) - refvA(ixO^S)*wCT(ixO^S,wAplus_))
    !w(ixO^S,wAplus_)  = w(ixO^S,wAplus_)  - qdt*(divv(ixO^S)*wCT(ixO^S,wAplus_)/2.0d0 + wCT(ixO^S,wAplus_)*Gamma_plus(ixO^S)    - refvA(ixO^S)*wCT(ixO^S,wAminus_) + refvA(ixO^S)*wCT(ixO^S,wAminus_))
    ...
    !>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    !> MAX: Adding reflection for 2D Kink waves analogous to AWs but with kink speed
    !>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    ...
    vk1(ixImin1:ixImax1,ixImin2:ixImax2) = abs(B(ixImin1:ixImax1,&
       ixImin2:ixImax2,1)/(dsqrt(w(ixImin1:ixImax1,ixImin2:ixImax2,&
       rho_)*(1+ff*zeta(ixImin1:ixImax1,ixImin2:ixImax2)-ff)**(-&
       1.d0))))*dsqrt(2/(zeta(ixImin1:ixImax1,ixImin2:ixImax2)+1))
    vk2(ixImin1:ixImax1,ixImin2:ixImax2) = abs(B(ixImin1:ixImax1,&
       ixImin2:ixImax2,2)/(dsqrt(w(ixImin1:ixImax1,ixImin2:ixImax2,&
       rho_)*(1+ff*zeta(ixImin1:ixImax1,ixImin2:ixImax2)-ff)**(-&
       1.d0))))*dsqrt(2/(zeta(ixImin1:ixImax1,ixImin2:ixImax2)+1))

    lnvk1(ixImin1:ixImax1,ixImin2:ixImax2) = log(vk1(ixImin1:ixImax1,&
       ixImin2:ixImax2))
    lnvk2(ixImin1:ixImax1,ixImin2:ixImax2) = log(vk2(ixImin1:ixImax1,&
       ixImin2:ixImax2))
    ...
    call gradient(lnvk1,ixImin1,ixImin2,ixImax1,ixImax2,ixOmin1,ixOmin2,&
       ixOmax1,ixOmax2,1,gradlnvk1)
    call gradient(lnvk2,ixImin1,ixImin2,ixImax1,ixImax2,ixOmin1,ixOmin2,&
       ixOmax1,ixOmax2,2,gradlnvk2)
    ...
    refvk(ixOmin1:ixOmax1,ixOmin2:ixOmax2) = 0.5d0*dsqrt(((((B(ixOmin1:ixOmax1,&
       ixOmin2:ixOmax2,1)/Bmag(ixOmin1:ixOmax1,&
       ixOmin2:ixOmax2))*w(ixOmin1:ixOmax1,ixOmin2:ixOmax2,&
       mom(1)))+vk1(ixOmin1:ixOmax1,ixOmin2:ixOmax2))*gradlnvk1(&
       ixOmin1:ixOmax1,ixOmin2:ixOmax2))**2.d0+((((B(ixOmin1:ixOmax1,&
       ixOmin2:ixOmax2,2)/Bmag(ixOmin1:ixOmax1,&
       ixOmin2:ixOmax2))*w(ixOmin1:ixOmax1,ixOmin2:ixOmax2,&
       mom(2)))+vk2(ixOmin1:ixOmax1,ixOmin2:ixOmax2))*gradlnvk2(&
       ixOmin1:ixOmax1,ixOmin2:ixOmax2))**2.d0)*(1+tanh((x(ixOmin1:ixOmax1,&
       ixOmin2:ixOmax2,2)-0.5d0)/0.1d0)) !*(1+tanh((x(ixOmin1:ixOmax1,ixOmin2:ixOmax2,2)-Ref_half_val)/width_ref)) !> Should be chosen to come on above TR

    !> Filter to check NanNs
    do ix1=ixOmin1,ixOmax1  
      do ix2=ixOmin2,ixOmax2
        if((ieee_is_nan(refvk(ix1,ix2))))then
          refvk(ix1,ix2) = 0.d0
          !write(*,*)"refvk = NaN at it = ", it
          !write(*,*)"x =                 ", x(ix1,ix2,1)
          !write(*,*)"x =                 ", x(ix1,ix2,2)
        end if
      end do
    ...
    !> Filter to check reflection for small values
    do ix1=ixOmin1,ixOmax1  
      do ix2=ixOmin2,ixOmax2
        if (refvk(ix1,ix2) > 1.d5)then
          !refvk(ix1,ix2)=0.d0
          write(*,*)"refvk > 1d5 at it = ", it
          write(*,*)"refvk =             ", refvk(ix1,ix2)
          write(*,*)"vk1 =               ", vk1(ix1-1:ix1:1,ix2-1:ix2+1)
          write(*,*)"vk2 =               ", vk2(ix1-1:ix1:1,ix2-1:ix2+1)
          write(*,*)"lnvk1 =             ", lnvk1(ix1-1:ix1:1,ix2-1:ix2+1)
          write(*,*)"lnvk2 =             ", lnvk2(ix1-1:ix1:1,ix2-1:ix2+1)
          !write(*,*)"x =                 ", x(ix1,ix2,1)
          !write(*,*)"x =                 ", x(ix1,ix2,2)
        end if
      end do
    ...
    do ix1=ixOmin1,ixOmax1
      do ix2=ixOmin2,ixOmax2
       w(ix1,ix2,wkminus_) = w(ix1,ix2,wkminus_) - qdt*(divv(ix1,ix2)*wCT(ix1,&
          ix2,wkplus_)/2.d0  + wCT(ix1,ix2,wkplus_)**(3.d0/2.d0)/(wCT(ix1,ix2,&
          rho_)*(1+ff*zeta(ix1,ix2)-ff)**(-1.d0))**0.5d0/Lperp(ix1,&
          ix2)  + refvk(ix1,ix2)*wCT(ix1,ix2,wkminus_) - refvk(ix1,&
          ix2)*wCT(ix1,ix2,wkplus_))
       w(ix1,ix2,wkplus_)  = w(ix1,ix2,wkplus_)  - qdt*(divv(ix1,ix2)*wCT(ix1,&
          ix2,wkminus_)/2.d0 + wCT(ix1,ix2,wkminus_)**(3.d0/2.d0)/(wCT(ix1,ix2,&
          rho_)*(1+ff*zeta(ix1,ix2)-ff)**(-1.d0))**0.5d0/Lperp(ix1,&
          ix2) - refvk(ix1,ix2)*wCT(ix1,ix2,wkminus_) + refvk(ix1,ix2)*wCT(ix1,&
          ix2,wkminus_))
      end do
    end do

    !w(ixO^S,wkplus_)  = w(ixO^S,wkplus_)  - qdt*(divv(ixO^S)*wCT(ixO^S,wkplus_)/2.d0  + wCT(ixO^S,wkplus_)**(3.d0/2.d0)/(wCT(ixO^S,rho_)*(1+ff*zeta(ixO^S)-ff)**(-1.d0))**0.5d0/Lperp(ixO^S) + refvk(ixO^S)*wCT(ixO^S,wkminus_) - refvk(ixO^S)*wCT(ixO^S,wkplus_))
    !w(ixO^S,wkminus_) = w(ixO^S,wkminus_) - qdt*(divv(ixO^S)*wCT(ixO^S,wkminus_)/2.d0 + wCT(ixO^S,wkminus_)**(3.d0/2.d0)/(wCT(ixO^S,rho_)*(1+ff*zeta(ixO^S)-ff)**(-1.d0))**0.5d0/Lperp(ixO^S) - refvk(ixO^S)*wCT(ixO^S,wkminus_) + refvk(ixO^S)*wCT(ixO^S,wkminus_))
  ...
  subroutine get_zeta(w,x,ixImin1,ixImin2,ixImax1,ixImax2,ixOmin1,ixOmin2,&
    ...
    zeta(ixImin1:ixImax1,ixImin2:ixImax2) = &
       (zeta0-1.d0)*exp(-(x(ixImin1:ixImax1,ixImin2:ixImax2,&
       1)-xprobmin1)/5.d0)+1.d0 !> MAX: Open field MAX TODO: Should we have a better representation for this expansion with distance along a field line rather than just height? (for 2D+)
