...
!! end rad cool
  ...
  subroutine w_add_source(qdt,ixI^L,ixO^L,wCT,w,x,energy,qsourcesplit,active)
    ...
    radius(ixO^S) = 1.d8/unit_length * (Busr/((B(ixO^S,1)**2.d0 + B(ixO^S,2)**2 + B(ixO^S,3)**2)**0.5d0))**0.5d0
  ...
  subroutine get_zeta(w,x,ixI^L,ixO^L,zeta)
    ...
    zeta(ixI^S) = (zeta0-1.d0)*exp(-(x(ixI^S,3)-xprobmin3)/(5.d0*(6.961d10/unit_length)))+1.d0 
